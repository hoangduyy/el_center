                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="zero_config_wrapper" class="dataTables_wrapper dt-bootstrap4">
                            <div class="card">
                                <form class="form-horizontal" action="{{route('be.room.update', ['room' => $data->id])}}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <div class="row">
                                            <div class="col-sm-6">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <input type="text" class="form-control" name="name" required placeholder="Nhập tên phòng học" value="{{ $data->name }}">
                                                    </div>
                                                </div>
                                                            ...
                                                            <select class="my-select2__select2 select2-wrapper" name="branch_id" required>
                                                                ...
                                                                @foreach($branch as $item)
                                                                    <option value = "{{ arrayGet($item, 'id') }}"
                                                                            {{ $data->branch_id == arrayGet($item, 'id') ? "selected" : '' }}>
                                                                        {{ arrayGet($item, 'name') }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
